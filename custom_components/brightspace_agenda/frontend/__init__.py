...
async def async_register(hass: HomeAssistant) -> None:
    ...
    # 1 - Chemin statique (cache_headers=False)
    ...
    if lovelace is None:
        _LOGGER.debug("Lovelace non disponible")
        return
    ...
    if not existing:
        ...
        _LOGGER.info("Ressource Lovelace enregistree : %s", versioned_url)
        return

    current_url = existing[0]["url"]
    if current_url == versioned_url:
        _LOGGER.debug("Ressource Lovelace a jour (%s)", versioned_url)
        return

    resource_id = existing[0]["id"]
    await resources.async_update_item(resource_id, {"res_type": "module", "url": versioned_url})
    _LOGGER.info("Ressource Lovelace mise a jour : %s -> %s", current_url, versioned_url)
